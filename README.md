# Sanad Biswas, Ph.D.

**Assistant Professor of Data Science**  
Sam Houston State University

I work at the intersection of **machine learning, statistical modeling, explainable and responsible AI, generative AI, predictive modeling, and MLOps**. My research focuses on building accurate, interpretable, reproducible, and practically useful data science systems.

[![Website](https://img.shields.io/badge/Website-sanadbiswas.github.io-0A66C2?style=flat-square)](https://sanadbiswas.github.io)
[![Google Scholar](https://img.shields.io/badge/Google%20Scholar-Profile-4285F4?style=flat-square&logo=google-scholar&logoColor=white)](https://scholar.google.com/citations?user=puRshxMAAAAJ&hl=en&oi=ao)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Sanad%20Biswas-0A66C2?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/sanad-biswas)
[![Email](https://img.shields.io/badge/Email-sxb218%40shsu.edu-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:sxb218@shsu.edu)

## Research Interests

- Machine Learning and Predictive Modeling
- Explainable AI and Feature Significance
- Responsible AI and Fairness
- Generative AI and Retrieval-Augmented Generation
- Statistical Inference and Model Validation
- MLOps and Reproducible Data Science

## Selected Research & Projects

### [CKD Temporal Screening](https://github.com/sanadbiswas/ckd-temporal-screening)
Temporal validation of a non-laboratory machine learning framework for chronic kidney disease screening in U.S. adults.

### [Feature Significance and Explainability in Neural Networks](https://github.com/sanadbiswas/Feature-Significance-and-Explainability-in-Neural-Network)
Research on feature significance, statistical testing, interpretation, and explainability in modern machine learning models.

### [Machine Learning](https://github.com/sanadbiswas/Machine_Learning)
Machine learning experiments, examples, and instructional materials covering applied modeling workflows.

## Publications & Patents

My research includes peer-reviewed work in machine learning, statistical modeling, explainable AI, and applied data science, as well as two published international patent applications.

- [Google Scholar Profile](https://scholar.google.com/citations?user=puRshxMAAAAJ&hl=en&oi=ao)
- [Personal Website — Publications & Patents](https://sanadbiswas.github.io/#patents)

## Teaching

I teach and have taught courses in:

- Statistical Methods for Data Science
- Data Science with Python
- Data Analytics with SQL and Python
- Statistical Methods in Practice
- Practical Data Analytics
- Enterprise AI Applications

## Technical Tools

**Languages:** Python, R, SQL  
**Machine Learning:** scikit-learn, XGBoost, LightGBM, PyTorch, TensorFlow  
**Data & Workflow:** pandas, NumPy, Jupyter, Git, GitHub, Docker  
**Research Interests:** XAI, Responsible AI, RAG, model validation, reproducibility

## Contact

- **Professional:** [sxb218@shsu.edu](mailto:sxb218@shsu.edu)
- **Personal:** [biswas.sanad@gmail.com](mailto:biswas.sanad@gmail.com)
- **LinkedIn:** [linkedin.com/in/sanad-biswas](https://www.linkedin.com/in/sanad-biswas)
- **Website:** [sanadbiswas.github.io](https://sanadbiswas.github.io)
